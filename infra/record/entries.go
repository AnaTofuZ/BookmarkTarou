...
var EntryWhere = struct {
	...
	ID:        whereHelperuint64{field: "`entries`.`id`"},
	URL:       whereHelper__byte{field: "`entries`.`url`"},
	Titile:    whereHelperstring{field: "`entries`.`titile`"},
	CreatedAt: whereHelpertime_Time{field: "`entries`.`created_at`"},
	UpdatedAt: whereHelpertime_Time{field: "`entries`.`updated_at`"},
}
...
func (o *Entry) Bookmarks(mods ...qm.QueryMod) bookmarkQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`bookmarks`.`entry_id`=?", o.ID),
	)
...
func (entryL) LoadBookmarks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeEntry interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`bookmarks`), qm.WhereIn(`bookmarks.entry_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Entry) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "record: unable to populate default values for entries")
	}
...
func (o *Entry) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "record: unable to upsert for entries")
	}
	...
	if err != nil {
		return errors.Wrap(err, "record: unable to retrieve unique values for entries")
	}
