...
func testUserToManyBookmarks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadBookmarks(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpBookmarks(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `Name`: `varbinary`, `Password`: `varbinary`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_           = bytes.MinRead
)
