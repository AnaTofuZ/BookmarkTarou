...
func testTagToManyBookmarkTagRelations(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TagSlice{&a}
	if err = a.L.LoadBookmarkTagRelations(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTagToManyAddOpBookmarkTagRelations(t *testing.T) {
	var err error
...
var (
	tagDBTypes = map[string]string{`ID`: `bigint`, `Content`: `varchar`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_          = bytes.MinRead
)
